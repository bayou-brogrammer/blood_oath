...
impl InventoryMode {
    pub fn new(world: &World) -> Self {
        ...
        let equipment = (&entities, &names, &glyphs, &equipped)
            .join()
            .filter(|(_, _, _, b)| b.owner == *player)
            .collect::<Vec<_>>();
                    ...
                    if matches!(key, VirtualKeyCode::E | VirtualKeyCode::A | VirtualKeyCode::D) =>
                {
                    if let Some(item_id) = self.inventory.get(self.inv_selection as usize) {
                        if let Some(inv_action) = InventoryAction::from_key(key) {
                            if InventoryAction::item_supports_action(world, item_id.0, inv_action) {
                                return (
                                    ModeControl::Push(
                                        InventoryActionMode::new(world, item_id.0, Some(inv_action)).into(),
                                    ),
                                    ModeUpdate::Update,
                                );
                            }
