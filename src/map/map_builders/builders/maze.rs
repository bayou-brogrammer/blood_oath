...
impl Grid {
    fn new(width: i32, height: i32) -> Grid {
        let mut grid = Grid { width, height, cells: Vec::new(), backtrace: Vec::new(), current: 0 };
    ...
    fn get_available_neighbors(&self) -> Vec<usize> {
        ...
        neighbor_indices
            .iter()
            .filter(|n| **n != -1 && !self.cells[**n as usize].visited)
            .for_each(|i| neighbors.push(*i as usize));
    ...
    fn find_next_cell(&mut self) -> Option<usize> {
        ...
        if !neighbors.is_empty() {
            if neighbors.len() == 1 {
                ...
                return Some(neighbors[(bo_utils::rng::roll_dice(1, neighbors.len() as i32) - 1) as usize]);
            }
        }
        None
    }
    ...
    fn copy_to_map(&self, map: &mut Map) {
        ...
        for cell in self.cells.iter() {
            ...
            if !cell.walls[TOP] {
                map.tiles[idx - map.width as usize] = GameTile::floor();
            }
            if !cell.walls[RIGHT] {
                map.tiles[idx + 1] = GameTile::floor();
            }
